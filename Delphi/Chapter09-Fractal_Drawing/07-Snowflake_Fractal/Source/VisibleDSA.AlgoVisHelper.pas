...
type
  ...
  public
    ...
    // 绘制文字
    class procedure DrawText(canvas: TCanvas; str: UString; x, y: integer);

    // 绘制三角形
    class procedure DrawTriangle(canvas: TCanvas; a, b, c: TPoint);
    // 填充三角形
    class procedure FillTriangle(canvas: TCanvas; a, b, c: TPoint);

    // 绘制直线
    class procedure DrawLine(canvas: TCanvas; a, b: TPointF);
  end;
...
class procedure TAlgoVisHelper.DrawLine(canvas: TCanvas; a, b: TPointF);
var
  path: TPathData;
begin
  canvas.Stroke.Color := _strokeColor;
  canvas.Stroke.Thickness := _strokeWidth;
  canvas.Stroke.Kind := _strokeStyle;
...
class procedure TAlgoVisHelper.DrawText(canvas: TCanvas; str: UString; x, y: integer);
var
  a: TRectF;
begin
  a := TRectF.Create(x, y, x + 80, y + 80);
  canvas.Fill.Color := _fillColor;

  if canvas.BeginScene then
  begin
    try
      canvas.FillText(a, str, False, 1, [], TTextAlign.Center);
    finally
      canvas.EndScene;
    end;
  end;
...
class procedure TAlgoVisHelper.DrawTriangle(canvas: TCanvas; a, b, c: TPoint);
var
  path: TPathData;
begin
  canvas.Stroke.Color := _strokeColor;
  ...
  path := TPathData.Create;
  try
    path.MoveTo(Point(a.X, a.Y));
    path.LineTo(Point(b.X, b.Y));
    path.LineTo(Point(c.X, c.Y));
    path.ClosePath;
...
class procedure TAlgoVisHelper.FillTriangle(canvas: TCanvas; a, b, c: TPoint);
var
  path: TPathData;
begin
  canvas.Fill.Color := _fillColor;
